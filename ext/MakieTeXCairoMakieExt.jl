...
# The following is old code for rendering, kept around for historical purposes.
...
function CairoMakie.draw_plot(scene::CairoMakie.Scene, screen::CairoMakie.Screen, primitive::MakieTeX.TeXImg)
    println("HELLO")
    if to_value(get(primitive, :visible, true))
        if isempty(primitive.plots)
            @warn "TexImg plots impty"
        else
            zvals = Makie.zvalue2d.(primitive.plots)
            for idx in sortperm(zvals)
                @info "Drawing plot type $(typeof(primitive.plots[idx]).parameters |> first)"
                CairoMakie.draw_plot(scene, screen, primitive.plots[idx])
            end
        end
    end
    return
