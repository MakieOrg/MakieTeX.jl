...
- Reworked MakieTeX around Makie 0.25's new `text_handler` hook (requires Makie ≥ 0.25) [#73](https://github.com/MakieOrg/MakieTeX.jl/pull/73).
  ...
  - **Breaking**: dropped `RENDER_DENSITY`, `CURRENT_TEX_ENGINE`, and `RENDER_EXTRASAFE`. GPU textures rasterize at the screen's `px_per_unit` times `MakieTeX.TEXTURE_RENDER_DENSITY[]` (default 2).
